...
def test_generate_coords_given_limb_lengths(train_coordinate_dataset, start, stop, version, plot=False):
    device = get_device()
...
if __name__ == "__main__":
    ...
    # d = test_generate_coords_given_limb_lengths(train_coordinate_dataset, START, STOP, VERSION, plot=False)

    # Plot original image
    plot_coordinates(train_coordinate_dataset.coordinates[START:STOP], train_coordinate_dataset.labels[START:STOP],
                     None)

    # Plot generated images conditioned on label and limb length
    for version in range(104, VERSION + 1, 52):
        test_generate_coords_given_limb_lengths(train_coordinate_dataset, START, STOP, version, plot=True)

    # Check if the mean differences from generated to input image decrease over time
    mean_differences = []
    for version in range(104, VERSION + 1, 52):
        generated_limb_lengths = test_generate_coords_given_limb_lengths(train_coordinate_dataset, START, STOP, version,
                                                                         plot=False)
